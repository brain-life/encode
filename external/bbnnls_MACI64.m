...
function out = bbnnls_MACI64(M, b, x0, opt)
...
function [f g] = funcGrad(A, b, x)    
    ...
    if (nargout > 1)
%     g = A'*Ax;
     g = Mtransp_times_b_mex(A.Phi.subs(:,1),A.Phi.subs(:,2),A.Phi.subs(:,3),A.Phi.vals,A.DictSig,reshape(Ax,[nTheta,nVoxels]),nFibers); % MEX default compiler version
