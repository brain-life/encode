function [fe, results] = FitFullModelSampleAllTracts(dwiFile, fgFileName, feFileName, L, p, n, alpha_v, alpha_f, lambda_a, lambda_r, fg_classification)
% INPUT
...
load(fg_classification);
ind_fibers_in_Tracts = find(classification.index~=0); % find fibers in All Tracts
ind_vox_tracts = feGet(fe,'vox ind from fibers',ind_fibers_in_Tracts);
nVoxels = length(ind_vox_tracts);

% subsampling of voxels
nVoxSample = round(n*nVoxels);
ind_vox_within_tracts = randperm(nVoxels,nVoxSample);

% determine indices of subsampled voxels
ind_vox = ind_vox_tracts(ind_vox_within_tracts);

disp('Sampled voxels: ', num2str(nVoxSample/feGet(fe,'nvoxels')))
...
Y = fe.life.diffusion_signal_img(ind_vox,ind_train)';
...
% save workspace to allow efficient parfor
%name = strcat(variables,num2str(round(rand*10000000)),'.mat');
%save(name);
%clear
%load(name,'Niter','threshold','Y','nTrain','s0','D','B','normY','name')
...
% Recover variables and overwrite B and s0 with obtained values
%B_new = B;
%load(name);
%B = B_new;
...
S0 = mean(fe.life.diffusion_S0_img(ind_vox,:),2);
[i,j,~] = find(C);
values = S0(i);
...
%Count = sparse(subCount(:,1),subCount(:,2),val,nVoxSample,nFibers); % Sparse matrix (nVoxSample x nFibers) containing the number of atoms per voxel per fiber

Count = double(sptenmat(Count,[1])); % Sparse matrix (nVoxSample x nFibers) containing the number of atoms per voxel per fiber
