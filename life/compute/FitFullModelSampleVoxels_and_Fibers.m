function [fe, results] = FitFullModelSampleVoxels_and_Fibers(dwiFile, fgFileName, feFileName, L, p, n, m, alpha_v, alpha_f, lambda_1, lambda_2, fg_classification)
% INPUT
...
% n:                    % of voxels randomly subsampled for fitting
% m:                    % of fibers randomly subsampled for fitting
...
%% Subsample fibers and find corresponding voxels
...
nFibers_in_Tracts = length(ind_fibers_in_Tracts);
nFibSample = round(nFibers_in_Tracts*m);

ind_fibers_in_Tracts_sub = ind_fibers_in_Tracts(randperm(nFibers_in_Tracts,nFibSample));
ind_vox_tracts = feGet(fe,'vox ind from fibers',ind_fibers_in_Tracts_sub); % get voxel indices for those fascicles
nVoxels = length(ind_vox_tracts);
nFibSample = length(ind_fibers_in_Tracts_sub);
...
disp(['Sampled fibers: ', num2str(100*nFibSample/feGet(fe,'nfibers')),'%, ', num2str(nFibSample), ' fibers'])
...
Phi = fe.life.M.Phi(:,ind_vox,ind_fibers_in_Tracts_sub); % Sparse Core tensor restricted to the selected voxels
...
out = bbnnls_orig(A, b, zeros(nFibSample,1), opt);
